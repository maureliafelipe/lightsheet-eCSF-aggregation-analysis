...
"""
lightsheet_demo_pipeline_with_preprocess.py

End-to-end demo pipeline for the light-sheet paper repository.

What this script can do
-----------------------
0. Preprocess full original stacks into lightweight subset stacks for the GitHub demo.
...
A. Edit ONLY the CONFIG section below:
   ...
   - voxel sizes
   - thresholds
   - selected Z slices for the lightweight demo data
   - example Z slices and display settings

B. Run the script:
       python lightsheet_demo_pipeline_with_preprocess.py
...
Expected inputs
---------------
1. Full original stacks in:
       INPUT_STACKS_DIR
   Expected examples:
       E1_cellmask_stack.tif
       E1_phalloidin_stack.tif
       E2_cellmask_stack.tif
       E2_phalloidin_stack.tif
       E3_cellmask_stack.tif
       E3_phalloidin_stack.tif
...
for _d in [DATA_DIR, RESULTS_DIR, QUANT_DIR, EXAMPLES_DIR]:
    _d.mkdir(parents=True, exist_ok=True)

EMBRYOS = ["E1", "E2", "E3"]

# Full original stacks (large files, not stored in GitHub repo)
INPUT_STACKS_DIR = Path("/Volumes/ANALYSIS_DK/Felipe/PhD paper/ANALISIS/All embryos z projection")

# Same Z used for both channels within each embryo
SELECTED_Z = {
    "E1": [100, 120, 140, 160],
    "E2": [100, 120, 140, 160],
    "E3": [20, 40, 60, 80],
...
# These are indices within the subset stacks, not the original full stacks.
Z_TO_SHOW = {
    "E1": 0,
    "E2": 0,
    "E3": 0,
...
csv_output = QUANT_DIR / "neuroepithelial_wall_vs_aggregated_network_metrics.csv"
absolute_metrics_path = QUANT_DIR / "neuroepithelial_wall_vs_aggregated_network_absolute_metrics.png"
fraction_metrics_path = QUANT_DIR / "neuroepithelial_wall_vs_aggregated_network_fraction_metrics.png"
...
# =========================================================
# STEP 0. PREPROCESS
# Create lightweight subset stacks for GitHub/demo analysis
# =========================================================
def preprocess_data_sample() -> Dict[str, Dict[str, Path]]:
    """
    Extract selected Z slices from the full original stacks and save
    lightweight subset stacks into DATA_DIR.

    Output files:
        E1_cellmask_subset.tif
        E1_phalloidin_subset.tif
        E2_cellmask_subset.tif
        E2_phalloidin_subset.tif
        E3_cellmask_subset.tif
        E3_phalloidin_subset.tif
    """
    DATA_DIR.mkdir(parents=True, exist_ok=True)

    input_files = {
        "E1": {
            "cellmask": INPUT_STACKS_DIR / "E1_cellmask_stack.tif",
            "phalloidin": INPUT_STACKS_DIR / "E1_phalloidin_stack.tif",
        },
        "E2": {
            "cellmask": INPUT_STACKS_DIR / "E2_cellmask_stack.tif",
            "phalloidin": INPUT_STACKS_DIR / "E2_phalloidin_stack.tif",
        },
        "E3": {
            "cellmask": INPUT_STACKS_DIR / "E3_cellmask_stack.tif",
            "phalloidin": INPUT_STACKS_DIR / "E3_phalloidin_stack.tif",
        },
    }
    ...
    for embryo in EMBRYOS:
        z_list = SELECTED_Z[embryo]

        print("\n========================================================")
        print(f"[preprocess] {embryo}")
        print("Selected Z:", z_list)
        print("========================================================")

        outputs[embryo] = {}

        for channel in ["cellmask", "phalloidin"]:
            in_path = input_files[embryo][channel]
            out_path = DATA_DIR / f"{embryo}_{channel}_subset.tif"

            if (not PREPROCESS_OVERWRITE) and out_path.exists():
                print(f"⏭️ Skipping existing subset: {out_path}")
                outputs[embryo][channel] = out_path
                continue

            if not in_path.exists():
                raise FileNotFoundError(f"Missing input stack: {in_path}")

            img = tiff.imread(in_path)

            if img.ndim != 3:
                raise ValueError(
                    f"Expected 3D stack (Z, Y, X), got {img.shape} for {in_path}"
                )

            z_max = img.shape[0] - 1
            bad_z = [z for z in z_list if z < 0 or z > z_max]
            if bad_z:
                raise ValueError(
                    f"Selected Z out of range for {embryo} {channel}: {bad_z} | valid range: 0-{z_max}"
                )

            subset = img[z_list].astype(np.float32)
            ...
            print("   dtype:", subset.dtype)
            print("   min/max:", float(subset.min()), float(subset.max()))

            outputs[embryo][channel] = out_path

    print("\n✅ All demo subset stacks created successfully.")
    print("Output folder:")
    print(DATA_DIR)
...
def main():
    # ---------------------------------
    # 0) preprocess lightweight GitHub demo stacks
    # ---------------------------------
    if RUN_PREPROCESS:
        preprocess_data_sample()

    # Keep track of outputs so the script can be run from scratch.
    cellmask_outputs: Dict[str, Dict[str, Path]] = {}
    phallo_outputs: Dict[str, Dict[str, Path]] = {}

    # ---------------------------------
    # 1) reconstruct probabilities + threshold
    # ---------------------------------
